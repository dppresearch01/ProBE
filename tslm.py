...
def testing_algo_one(count,threshold,a,l_sensitivity,epsilonMax):
    b = 0.05
    a= 200
    selected,ep = threshold_shift(count,[threshold]*len(count), a,b,epsilonMax,l_sensitivity)
    tp,tn,fp,fn= classify(count,[threshold]*len(count),selected)
    fpr = len(fp)/len([i for i in count if i <=threshold])
    fnr = len(fn)/len([i for i in count if i > threshold])
